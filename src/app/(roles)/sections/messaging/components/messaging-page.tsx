...
const MessagingPage = () => {
	...
	return (
		<div className=" h-full max-h-[calc(100vh-135px)] flex-1 -mb-2.5 text-hub-black flex gap-x-1">
			<Card className="w-[400px] flex flex-col overflow-y-hidden">
				<CardHeader className="pb-2">
					<h2 className="font-semibold ml-1 mb-1">Chats</h2>
					<div className="relative bg-hub-grey rounded">
					...
					{arr.map((item, index) => (
						...
						>
							...
							<div className="flex-1 grid grid-cols-[auto_1fr] gap-y-2 gap-x-2">
								<h3 className="h-fit font-semibold flex items-center gap-x-2 after:block after:size-2 after:rounded-full after:bg-hub-blue">
									Dr. Dolor Manchi
								</h3>
								...
								<CheckCheck className="justify-self-end size-5 text-hub-blue" />
							</div>
						</div>
					))}
				</CardContent>
