...
const formSchema = z.object({
  ...
  password: z.string().min(8, { message: "Password must be at least 8 characters.", }).regex(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])/, {
...
const passwordFormats: Array<{
  ...
  {id: 'number & symbol', label: "Least one number (0-9) or symbol", regex:/\d/},
  {id: 'textcase', label: "Lowercase (a-z) and uppercase (A-Z)", regex:/({a-z}), ({A-Z})/}
  ]
...
const PassStrengthAnimation: React.FC<PassStrengthAnimationProps> = ({ password }) => {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <div className='relative'>
                      ...
                      <Button 
                        ...
                        variant='ghost' 
                        size="icon" 
                        className='absolute right-0 top-0 h-full px-3 py-2'   onClick={() => setShowPassword(!showPassword)}
                      >
                        {showPassword ? (
                  ...
                  <FormMessage />
                  <PassStrengthAnimation password={password} />
                </FormItem>
              )}
            />
